...
void CodeEditor::onCursorPositionChanged(){
    ...
    int line = int(this->textCursor().blockNumber()+1);
    int column = int(this->textCursor().columnNumber()+1);
    //qDebug()<<"CodeEditor::onCursorPositionChanged() - "<< line;// "Line: "<<this->textCursor().blockNumber()+1;
    qDebug()<<"CodeEditor::onCursorPositionChanged() - Column: "<<column;
    if (_oldLine != line) {
        _oldLine = line;
        qDebug()<<"CodeEditor::onCursorPositionChanged() - Line changed! ("<<line<<")";
    }
}
